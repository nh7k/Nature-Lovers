...
const EMAIL_CONFIG = {
    ...
    recipientEmail: 'heerasaw14@gmail.com'
};
...
async function sendRegistrationEmail(form) {
    ...
    try {
        ...
        const templateId = window.EMAILJS_REGISTRATION_TEMPLATE_ID || EMAIL_CONFIG.templateIds.registration;
...
async function sendContactEmail(form) {
    ...
    try {
        ...
        // Send email via EmailJS
        const serviceId = window.EMAILJS_SERVICE_ID || EMAIL_CONFIG.serviceId;
        const templateId = window.EMAILJS_CONTACT_TEMPLATE_ID || EMAIL_CONFIG.templateIds.contact;
...
function sendContactEmailFallback(formData) {
    const subject = encodeURIComponent(`Nature Lovers Contact: ${formData.get('subject')}`);
...
function checkEmailServiceStatus() {
    return {
        ...
        publicKey: !!(window.EMAILJS_PUBLIC_KEY || EMAIL_CONFIG.publicKey !== 'YOUR_EMAILJS_PUBLIC_KEY'),
        serviceId: !!(window.EMAILJS_SERVICE_ID || EMAIL_CONFIG.serviceId),
        templates: {
            registration: !!(window.EMAILJS_REGISTRATION_TEMPLATE_ID || EMAIL_CONFIG.templateIds.registration),
            contact: !!(window.EMAILJS_CONTACT_TEMPLATE_ID || EMAIL_CONFIG.templateIds.contact)
        }
    };
}
